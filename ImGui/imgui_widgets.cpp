...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        int subdivisionsY = 10;
        ImVec2 sub_quad_size = ImVec2(sv_picker_size / subdivisionsX, sv_picker_size / subdivisionsY);
        for (int y = 0; y < subdivisionsY; ++y)
        {
            for (int x = 0; x < subdivisionsX; ++x)
            {
                float saturations[2];
                saturations[0] = float(x) / float(subdivisionsX);
                saturations[1] = float(x + 1) / float(subdivisionsX);

                float values[2];
                values[0] = float(subdivisionsY - y) / float(subdivisionsY);
                values[1] = float(subdivisionsY - y - 1) / float(subdivisionsY);
        ...
        //draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, col_black, col_black);
